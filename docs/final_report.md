...
**A genuine gap against the assignment's own data spec (section "Assignment", line 7): camera angle.** The assignment states the real-time flight provides telemetry "without GNSS" — implying GNSS is the only field withheld, and that barometric height and camera angle remain available. We honour that for altitude: `rel_alt_m`, the barometer-derived relative altitude from the query SRT, is read every frame and is never GPS-derived. But camera angle is **not** actually read from the query telemetry at inference — it is a hardcoded constant (`--camera-angle-source fixed`, 60° for Mini 3 Pro / 45° for Air 3/3S), because neither drone's SRT file logs gimbal pitch (confirmed: `gimbal_pitch` is parsed for in `src/telemetry_parser.py`'s schema but never populated from the actual DJI SRT format for these models). The code path that would read a live `gimbal_pitch` value (`select_camera_angle()` in `src/project_ground_point.py`) exists and is exercised by the `--camera-angle-source gimbal_pitch` flag, but is dead in practice for every flight in this dataset. This is a real, disclosed mismatch between what the assignment's data description promises and what the actual hardware logs — not a shortcut we chose, but a constraint of the specific drones flown. Heading/yaw, by contrast, is not listed in the assignment's telemetry fields at all, so estimating it causally from the query's own GPS trajectory (section 3, "Limitations" — note this is the one disclosed GNSS-adjacent exception) is an addition the assignment doesn't require us to avoid, not a violation of a stated guarantee.
...
**Honest caveat on the "real-time" claim:** the *mean* throughput (1.99-2.17 fps) is faster than the 1 fps the source video is sampled at, so on average the pipeline keeps up with — and gets ahead of — the incoming stream. But this is a mean, not a per-frame guarantee: individual VPR-fallback frames take noticeably longer than the satellite-only path (mean ≈0.75-0.91 s vs ≈0.20-0.41 s), and the single worst frame observed was 2.48 s on v13 and 1.21 s on v14 — both above the 1-second arrival interval of a true 1 fps live stream. The current script processes strictly sequentially with no frame-dropping or buffering logic, so a live deployment hitting several consecutive slow VPR-fallback frames would fall behind and need either a frame-skip policy or a worker queue; this was not implemented or tested here. The 1 fps sampling rate itself is also a simplification chosen for this benchmark (the underlying DJI footage is 24-30 fps) — the system was not tested against the original raw frame rate.
